  ...
  <div class="slog-content-box">
    <div class="content-box-header">
      ...
      <div class="content-box-header-inner-right">
        {#if article.userId === $auth.id}
          ...
          <div class="drop-menu-box block" class:block={isViewMenu}>
            <ul>
              <li>
                <button
                  on:click={() => onEditModeArticle(article.id)}
                  class="drop-menu-button">수정</button
                >
              </li>
              <li>
                <button
                  on:click={() => onDeleteArticle(article.id)}
                  class="drop-menu-button">삭제</button
                >
              </li>
            </ul>
          </div>
    ...
    <div class="content-box-bottom">
      <div class="button-box-inner-left">
        {#if article.likeMe}
          <button on:click={() => onCancelLike(article.id)} class="flex">
            <svg
        ...
        {:else}
          <button on:click={() => onLike(article.id)} class="flex">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
              class="w-6 h-6 mr-1"
